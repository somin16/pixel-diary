...
class DiaryAIGenerateView(APIView):
    """AI 그림 생성(임시 저장) API"""
    ...
    def post(self, request):
        ...
        - positive_prompt, negative_prompt, diary_frame_item_id 받기
        - diary_frame_item_id 인벤토리 보유 확인
        - ComfyUI로 이미지 생성
        - Supabase Storage에 임시 업로드 ({user_id}/preview.png)
        - al_image 테이블에 저장 (diary_id=NULL)
        ...
        try:
            ...
            # 4. Supabase Storage에 임시 업로드 (upsert로 덮어쓰기)
            ...
            # 5. 기존 임시 row 삭제 후 al_image 저장 (재생성 시 고아 데이터 방지)
            requests.delete(
                f"{supabase_url}/rest/v1/al_image?user_id=eq.{user_id}&diary_id=is.null",
                headers=headers,
            )

            al_image_response = requests.post(
                f"{supabase_url}/rest/v1/al_image",
                headers={**headers, "Prefer": "return=representation"},
                json={"user_id": user_id, "image_url": image_url},
            )
            if al_image_response.status_code not in [200, 201]:
                raise Exception(f"al_image 저장 오류: {al_image_response.text}")

            image_id = al_image_response.json()[0].get("id")
