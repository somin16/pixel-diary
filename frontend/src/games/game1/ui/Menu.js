...
export function createMenu(scene) {
    ...
    // 돌아가기 버튼
    const returnButton = scene.add.nineslice(centerX, centerY - centerY / 8, "menu_window_button", 0, buttonWidth, buttonHeight, 2,2,2,2);
    returnButton.setInteractive();   // 클릭가능여부(없으면 클릭이 안됩니다)
    returnButton.setScrollFactor(0); // 가운데 고정
    returnButton.setScale(2);        // 해상도x2

    // 돌아가기 텍스트
    const returnText = scene.add.text(centerX , centerY - centerY / 8, "돌아가기", {
        fontFamily: "Mona",
        fontSize: "18px",
        fill: "#ffffff"
    ...
    // 게임종료 버튼
    const gameEndButton = scene.add.nineslice(centerX, centerY + centerY / 3, "menu_window_button", 0, buttonWidth, buttonHeight, 2,2,2,2);
    gameEndButton.setInteractive();
    gameEndButton.setScrollFactor(0);
    gameEndButton.setScale(2);

    // 게임종료 텍스트
    const gameEndText = scene.add.text(centerX , centerY + centerY / 3, "게임종료", {
        fontFamily: "Mona",
        fontSize: "18px",
        fill: "#ffffff"
    }).setOrigin(0.5, 0.5); // 가운데 정렬
    ...
    // 소리 온오프 버튼
    const soundBtn = scene.add.image(centerX + 100, centerY - centerY / 2, "sound_icon_on");
