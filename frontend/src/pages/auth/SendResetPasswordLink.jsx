...
export default function SendResetPasswordLink() { // 비밀번호 재설정 링크 이메일로 보내는 페이지 내보내기
    ...
    // [상태] 피드백 메시지
    const [emailStatus, setEmailStatus] = useState({ state: 'defalut', message: '' });
    ...
    useEffect(() => {
        const checkEmail = async () => {
            if (debouncedUserEmail) {
                ...
                setEmailStatus({ state: 'defalut', message: '' })
            }
        };
        checkEmail();
    }, [debouncedUserEmail]);
    ...
    const handleSendEmail = async (e) => {
        ...
        try{
            ...
            if(response.ok) {
                ...
                setEmailStatus({ // 서버 에러 메시지 표시
                    state: 'error',
                    message: authData.message || "서버 오류로 인해 발송 실패. 다시 시도해 주세요."
                });
            }
        } catch (error) {
    ...
    return(
        // 전체 컨테이너
        <div className='w-full h-full items-center flex flex-col p-[5vw]'>
            {/* 비밀번호 재설정 글씨 */}
            <h1 className='text-[3vw] font-bold text-center mt-[1vw]'>Pixel Diary</h1><br/>
            <h1 className='text-[1.7vw] font-bold text-center mb-[4vw]'>비밀번호 재설정</h1>
            {step === 1 ? (
                /* STEP 1 : 이메일 입력 화면 */
            <form onSubmit={handleSendEmail} noValidate className='w-full flex flex-col'>
                ...
                /* STEP 2 : 이메일 발송 완료 화면 */
                <div className="w-full flex flex-col items-center gap-[1vw] text-center">
                    
                    
                    {/* 입력했던 이메일을 강조해서 보여줌 */}
                    <div className="bg-gray-50 p-4 rounded-lg w-full border border-dashed border-gray-300">
                        <span className="text-blue-500 font-normal text-[1.2vw] break-all">{user_email}</span>
                    </div>

                    <p className="font-bold text-[1.2vw] text-gray-800">메일이 발송되었습니다!</p>
                    <p className="text-[0.8vw] text-gray-500 leading-relaxed">
                        메일함에 도착한 재설정 링크를 클릭하여<br />
                        비밀번호 변경을 완료해 주세요.
                    </p>
