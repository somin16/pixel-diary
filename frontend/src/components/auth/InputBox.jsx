 ...
 * @property {'success' | 'error' | 'default'} state - 유효성 상태
 * @property {string} message - 표시할 안내 문구
 */

/**
 * [auth 컴포넌트] 로그인 및 회원가입 전용 픽셀아트 입력창
 * * @param {object} props
 * @param {string} props.label - 입력창 왼쪽 상단에 표시될 이름 (이메일, 비밀번호 등)
 * @param {string} props.type - input의 타입 (text, email, password 등)
 * @param {string} props.placeholder - 미입력 시 보여줄 안내 문구
 * @param {string} props.value - 현재 입력된 값
 * @param {function} props.onChange - 값 변경 시 실행될 핸들러 함수
 * @param {object} props.status - 유효성 검사 상태 ({ state: 'success'|'error', message: '...' })
 * @param {string} props.currentTheme - 앱의 현재 테마 (라이트/다크 등)
 */
